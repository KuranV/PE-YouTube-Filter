...
**Detection.** `content.js` runs on every `youtube.com` page, walks all video cards (`ytd-rich-item-renderer`, `ytd-video-renderer`, `ytd-compact-video-renderer`, `ytd-grid-video-renderer` and a few others), and extracts the channel ID from `/channel/UC...` anchors. It prefers ID matches but falls back to channel-name matches when YouTube serves `/@handle` URLs. A `MutationObserver` with a 200 ms debounce handles lazy-loaded cards, and `yt-navigate-finish` resets state on SPA navigation.

**Modes.** *Hide entirely* sets `display:none` on the card. *Show with label* attaches a red badge to the thumbnail. *Show normally* removes any marks and stops processing. Switching modes in the popup messages the active tab; no reload needed.
